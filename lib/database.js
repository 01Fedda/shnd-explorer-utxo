    ...
    if (address) {
      ...
      if ( hash == 'coinbase' ) {
		...
		console.log("vin:%s,n:%d,from:%s",txid,n,script);
              while ((indexrm = unspent.findIndex(function(element,index,array){return element.tx_hash===script && element.tx_ouput_n===n})) !== -1) {
...
//		console.log("vout:%s,n:%d",txid,n);
            unspent.push({tx_hash:txid,value:amount,tx_ouput_n:n,script:script,blockheight:blockheight});
          }
          if (unique == true) {
      ...
      if (type == 'vin') {
        ...
        var newAddress = new Address({
          ...
          unspent: [ {tx_hash:txid,value:amount,tx_ouput_n:n,script:script,blockheight:blockheight} ],
          received: amount,
          balance: amount,
        });
      }
...
//	var rawtx = new Buffer(tx.hex, "hex");
//	console.log("sending rawtx", rawtx, tx.hex);
	//return;
